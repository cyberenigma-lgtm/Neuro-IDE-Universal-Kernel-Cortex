...
class Plugin(NeuroModule):
    ...
    def run_scan(self):
        ...
        for i in range(total_blocks):
            ...
            if leaks_detected > 0 and i % (total_blocks // (leaks_detected + 1) + 1) == 0:
                self.memory_state[i] = 3 # Leak
                sl = alloc_lines[leak_idx % len(alloc_lines)] if alloc_lines else "Unknown"
                self.block_details[i] = f"Source: L{sl}"
                leaks_detected -= 1
                leak_idx += 1
            elif rand > 0.8: 
                self.memory_state[i] = 1 
                self.block_details[i] = "Status: Active Payload"
            elif rand > 0.7: 
                self.memory_state[i] = 2 
                self.block_details[i] = "Status: Reserved Metadata"
            else: 
